...
//添加一些干扰点
for($i = 0; $i < 25; ++ $i){
    imagesetpixel($img, mt_rand(0,150), mt_rand(0,50), RandomColor($img));
}
...
for($i = 0; $i < 4; ++ $i){
    imagettftext($img, $fontSize, mt_rand(-15, 15), $x, $y, RandomColor($img), '/usr/share/fonts/truetype/ubuntu/Ubuntu-B.ttf', $code[$i]);
    $x += $dx;
}
